...
void parameters(std::map<std::string, unsigned> &configU, std::map<std::string, double> configD)
{
	...
	std::string benchTypes[] = {"UNIFORM", "SKEW", "CLUSTER", "CALIFORNIA", "BIOLOGICAL", "FOREST", "CANADA", "MICROSOFTBUILDINGS"};
	...
	while ((option = getopt(argc, argv, "t:m:a:b:n:s:r:v:")) != -1)
	{
		switch (option)
		{
			...
			default:
			{
				...
				std::cout << "    -m  Specifies benchmark type {0 = Uniform, 1 = Skew, 2 = Clustered, 3 = California, 4 = Biological, 5 = Forest, 6 = Canada, 7 = MICROSOFTBUILDINGS}" << std::endl;
