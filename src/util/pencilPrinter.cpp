...
void PencilPrinter::printToPencil(nirtree::Node *root)
{
	// Print directory
	std::string printDirectory = "prints/" + pencilIdGenerator();
	mkdir(printDirectory.c_str(), 0777);
	...
	for (;explorationQ.size();)
	{
		...
		for (unsigned i = 0; i < currentContext.first->branches.size(); ++i)
		{
			...
			fillPencilRectangle(currentPage, currentContext.first->branches[i].boundingBox, boundingBoxColour, currentPolygon);
...
void PencilPrinter::printToPencil(rplustree::Node *root)
{
	// Print directory
	std::string printDirectory = "prints/" + pencilIdGenerator();
	mkdir(printDirectory.c_str(), 0777);
