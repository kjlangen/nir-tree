...
float Rectangle::computeExpansionArea(Rectangle requestedRectangle) {
    // Expanded rectangle area computed directly
    float expandedArea = fabs(
        (fmin(requestedRectangle.lowerLeft.x, lowerLeft.x) - fmax(requestedRectangle.upperRight.x, upperRight.x)) *
...
void Rectangle::expand(Rectangle givenRectangle) {
    ...
    upperRight = Point(fmax(givenRectangle.upperRight.x, upperRight.x),
                       fmax(givenRectangle.upperRight.y, upperRight.y));
}
...
std::vector<Rectangle> Rectangle::fragmentRectangle(Rectangle clippingRectangle) {
    ...
    if (containsPoint(clippingRectangle.upperRight)) {
        ...
        v.push_back(
            Rectangle(clippingRectangle.upperRight.x, lowerLeft.y, upperRight.x, clippingRectangle.upperRight.y));
    }
